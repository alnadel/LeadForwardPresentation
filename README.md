...
There are two versions, and both use the same keys and speaker view:
...
## Present it
1. Double-click `Present/Present (Windows).bat` (or `Present (Mac).command`). The deck opens full screen, on black, waiting for you.
2. Press **S** to open the speaker view and drag it to your laptop screen. Then press any key to begin. (Opened directly, the HTML goes full screen on that first key; **F** toggles full screen at any time. v1 has no start screen: it begins as soon as it opens.)
