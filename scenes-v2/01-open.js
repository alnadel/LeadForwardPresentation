...
Deck.scene({
  ...
  html: `
    ...
    <div class="op-birth"><i class="op-bloom"></i><i class="op-flare"></i>${Array.from({ length: 12 }, (_, i) => `<b style="--a:${i * 30 + 8}deg;--r:${120 + (i * 47) % 90}px;--k:${i}"></b>`).join('')}</div>
  ...
  step(n, prev, ctx) {
    ...
    if (n === 1 && !ctx.instant) {
      ctx.after(1150, () => window.Field && Field.burst(159, 912, { radius: 2400, dur: 3.4 }));
    }
  },
});
